...
class DataLoaderBase(object):
    ...
    def __init__(self, args):
        ...
        self.train_file = os.path.join(self.data_dir, 'train1.txt')
        self.val_file = os.path.join(self.data_dir, 'valid1.txt')
        ...
        self.statistic_cf()
        self.item_entities, self.item_relations = self.load_item_entities()
    
    def load_item_entities(self):
        # build a dictionary to store entities of items
        kg_data = self.load_kg(self.kg_file)
        item_entities = collections.defaultdict(set)
        item_relations = collections.defaultdict(set)
        for row in kg_data.loc[kg_data['h'] < self.n_items].iterrows():
            h, r, t = row[1]
            item_entities[h].add(t)
            item_relations[h].add(r)
            
        for row in kg_data.loc[kg_data['t'] < self.n_items].iterrows():
            h, r, t = row[1]
    ...
    def load_cf(self, filename):
        ...
        for l in lines:
            ...
            if len(inter) > 1:
                ...
                item_ids = torch.tensor(list(set(item_ids)))
    ...
    def statistic_cf(self):
        self.n_users = max(max(self.cf_train_data[0]), max(self.cf_val_data[0]), max(self.cf_test_data[0])) + 1
        self.n_items = max(max(self.cf_train_data[1]), max(self.cf_val_data[1]), max(self.cf_test_data[1])) + 1
    ...
    def load_kg(self, filename):
        if (self.data_name == 'book') or (self.data_name =='music_small') or (self.data_name =='movie'):
